      ...
      con.query(myquery1, function(err, result1, fields) {
        if (err)
          ...
          con.query(myquery2, function(err, result2, fields) {
            if (err)
              res.status(500).send({"status" : "failed"});
            else if (result2.length == 0) { // sta protypa ton ypoloipon episis
                                            // den exei logo yparxis
              res.status(402).send({"status" : "failed"});
            } else {
              var [x] = result1;
              var [y] = result2;
              if (x.Charge > y.Charge) {
                total = x.Charge - y.Charge
              } else {
                total = y.Charge - x.Charge
                z = op1_ID
                op1_ID = op2_ID
                op2_ID = z
              }
...
router.get('/Debts/:op1_ID/:op2_ID/:date_from/:date_to', debts);
