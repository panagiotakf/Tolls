...
function funGet(req, res) {
  res.render(path.join(__dirname + '/frontend-Debts.ejs'));
};
...
function funPost(req, res) {
  ...
  if (format == "on") {
    res.redirect('https://localhost:9103/interoperability/api/Debts/' + op_1 + '/' + op_2
...
router.get('/Debts', funGet)
router.post('/Debts', funPost)
